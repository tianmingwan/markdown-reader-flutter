...
| 选中菜单里的「问 DeepSeek」（新对话并直接提问）/「放进 DeepSeek」（只放进输入框） | ✅ |

**「选中内容 → 内置 DeepSeek」的两种用法**（`AppState.askAi`）：
...
**顺带清掉了系统/第三方塞进选中菜单的"文本处理"项**：安卓上 `ACTION_PROCESS_TEXT`
的处理器（联想 ROM 的"智能识别"、爱奇艺搜索、朗读、在 Via 中搜索、搜视频…）会被 Flutter
以 `ContextMenuButtonType.custom` 的形式并进选中菜单。`filteredSelectionMenuItems()`
把它们全部过滤掉，只保留复制/分享/全选 + 我们的两项（真机实测：菜单里只剩这 5 项）。
